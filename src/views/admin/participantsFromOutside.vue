<template>
  <div class="container my-5">
    ...
    <div class="conference-header text-center mb-5">
      <h1 class="conference-title">{{ $t('participantsList') }}</h1>
    </div>
    ...
    <div class="search-filters mb-4">
      <div class="row g-3">
        ...
        <div class="col-md-6">
          <b-form-group :label="$t('country')" label-for="searchCountry">
            <b-form-select
              id="searchCountry"
              v-model="searchFilters.country"
              :options="countries"
              @change="debouncedFetchParticipants"
            ></b-form-select>
          </b-form-group>
        </div>
      </div>
      ...
      <b-card-body>
        ...
        >
          ...
          <template #cell(passport)="row">
            <a v-if="row.item.passportFullUrl" :href="row.item.passportFullUrl" target="_blank" class="btn btn-sm btn-outline-primary">
              {{ $t('viewPassport') }}
            </a>
          ...
          <template #cell(notes)="row">
            {{ row.item.arNote || row.item.enNote || row.item.peNote || '-' }}
          </template>
    ...
    <b-modal v-model="showEditModal" :title="$t('editParticipant')" size="lg" @ok="handleSave" :ok-title="$t('save')" :cancel-title="$t('cancel')">
      <b-form @submit.prevent="handleSave">
        <div class="row g-3">
          ...
          <b-form-group class="col-md-6" :label="$t('country') + ' *'" label-for="country">
            <b-form-select
              id="country"
          ...
          <b-form-group class="col-md-6" :label="$t('ThePassport') + ' *'" label-for="passportFile">
            <b-form-file
              id="passportFile"
              v-model="editForm.passportFile"
              accept=".pdf,.png,.jpg,.jpeg"
              :state="formErrors.passportFile ? false : null"
              @change="onPassportFileChange"
            ></b-form-file>
            <a v-if="editForm.passportFullUrl" :href="editForm.passportFullUrl" target="_blank" class="mt-2 d-block">{{ $t('viewCurrentPassport') }}</a>
            <b-form-invalid-feedback v-if="formErrors.passportFile">{{ formErrors.passportFile }}</b-form-invalid-feedback>
          </b-form-group>
          <b-form-group class="col-md-6" :label="$t('powerPoint') + ' *'" label-for="powerPointFile">
            <b-form-file
              id="powerPointFile"
...
export default {
  ...
  computed: {
    tableFields() {
      return [
        ...
        { key: 'passport', label: this.$t('ThePassport') },
  ...
  methods: {
    async fetchParticipants() {
      this.isLoading = true;
      try {
        const response = await axios.get('https://api.int-mgm.org/api/OutsideIraq/GetAllInformationOutsideIraq', {
          params: {
        ...
        this.tableItems = response.data.items.map(item => ({
          ...
          note: item.arNote || item.enNote || item.peNote || '-',
        }));
        this.pagination = {
    ...
    onPassportFileChange(event) {
      ...
      if (file && !['application/pdf', 'image/jpeg', 'image/png'].includes(file.type)) {
        this.formErrors.passportFile = this.$t('invalidFileExtension');
        this.editForm.passportFile = null;
        event.target.value = '';
      } else {
        this.formErrors.passportFile = '';
        this.editForm.passportFile = file;
      }
    },
    ...
    openEditModal(item) {
      this.editForm = {
        ...
        passportFile: null,
        passportFullUrl: item.passportFullUrl || '',
        powerPointFile: null,
    ...
    async handleSave(bvModalEvt) {
      ...
      if (this.editForm.passportFile) formData.append('ThePassport', this.editForm.passportFile);
      if (this.editForm.powerPointFile) formData.append('PowerPoint', this.editForm.powerPointFile);
      if (this.$i18n.locale === 'ar') formData.append('ArNote', this.editForm.note || '');
      if (this.$i18n.locale === 'en') formData.append('EnNote', this.editForm.note || '');
      if (this.$i18n.locale === 'fa') formData.append('PeNote', this.editForm.note || '');
      formData.append('Country', this.editForm.country || '');
      formData.append('ConferenceVersionID', this.editForm.conferenceVersionID);

      try {
        await axios.put(`https://api.int-mgm.org/api/OutsideIraq/UpdateInformationOutsideIraq?id=${this.editForm.id}`, formData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
    ...
    validateForm() {
      ...
      if (!this.editForm.passportFullUrl && !this.editForm.passportFile) {
        this.formErrors.passportFile = this.$t('requiredField');
        isValid = false;
      }
    ...
    async handleDelete() {
      try {
        await axios.delete(`https://api.int-mgm.org/api/OutsideIraq/DeleteInformationOutsideIraq?id=${this.deleteItem.id}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
