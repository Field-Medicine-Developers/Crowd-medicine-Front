<template>
  <div class="container mt-4 cb" dir="rtl">
    <h1 class="text-center mb-4">اقسام المدونة</h1>
    <div class="card">
      <div class="card-body">
...
export default {
  ...
  methods: {
    async fetchcategories() {
      try {
        const response = await api.get(
          `Category/GetAllCategories?PageNumber=${this.currentPage}&PageSize=${this.pageSize}`
        );
    ...
    async addCategories() {
      ...
      formData.append("ArabicName", this.form.arabicName);
      formData.append("EnglishName", this.form.englishName);
      formData.append("PersianName", this.form.persianName);
    ...
    async updateCategories() {
      ...
      try {
        await api.put(
          `Category/UpdateCategory?id=${this.editForm.id}`,
          this.editForm
        );
        this.fetchcategories();
